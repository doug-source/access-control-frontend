...
describe('useRemotionHandler hook', () => {
    it('runs returning the first callback correctly', () => {
        ...
        const { result } = renderHook(
            () => {
                return useRemotionHandler((user) => Boolean(user));
            },
            { wrapper }
        );
    ...
    it('runs triggering the remotion when the users are different correctly', () => {
        const onRemove = vi.fn();
        const dispatch = vi.fn();
        const wrapper = ({ children }: Children) => {
            return createElement(RemotionDataProvider, {
        ...
        const { result } = renderHook(
            () => {
                ...
                return useRemotionHandler((_) => false);
            },
            { wrapper }
        );
        const user = {
    ...
    it('runs triggering the remotion with remotionConfirm as false', () => {
        const onRemove = vi.fn();
        const dispatch = vi.fn();
        const wrapper = ({ children }: Children) => {
            return createElement(RemotionDataProvider, {
