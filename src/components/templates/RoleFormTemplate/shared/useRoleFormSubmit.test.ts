...
import { useRoleFormSubmit } from './useRoleFormSubmit';
...
describe('useRoleFormSubmit hook', () => {
    beforeAll(() => {
    ...
    it('runs requesting and returning the status code equals 201 correctly', async () => {
        ...
        result.current[0].current = {} as unknown as HTMLInputElement;
        await result.current[1](makeFormEvent());
        expect(dispatch).toHaveBeenCalledWith({ type: 'loading' });
        expect(dispatch).toHaveBeenCalledTimes(1);
        expect(onNavigate).toHaveBeenCalledWith('/roles', { replace: true });
    });
