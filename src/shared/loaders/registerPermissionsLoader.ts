...
import { HttpSuccessResponse } from '../types/Http/Response';
...
export const registerPermissionsLoader = (
    pageRequester: PageRequester,
    token: string,
    navigation: ListNavigations['registerPermission'],
    endpoint: RegisterPaginations['permission'],
    keyContext: PaginateKeyContext,
    id: number
) => {
    return ({
        request,
    }: LoaderFunctionArgs): PaginationLoaderData<RegisterPermissionIndex> => {
        ...
        if (searchParams.has('email') && searchParams.size === 1) {
            ...
            output: pageRequester
                ...
                .then((output) => {
                    const { body } = output as HttpSuccessResponse<
                        Pagination<RegisterPermissionIndex>,
                        200
                    >;
                    return {
                        data: body.data,
